...
public class ObjectOrderedSet<T> extends ObjectSet<T> implements Ordered<T>, Serializable {
	 ...
	 * use {@link OrderedSetIterator#OrderedSetIterator(ObjectOrderedSet)} to nest iterators.
	 *
	...
	public static class OrderedSetIterator<K> extends ObjectSetIterator<K> {
		...
		public OrderedSetIterator (ObjectOrderedSet<K> set) {
			super(set);
			items = set.items;
		}
	...
	public static <T> ObjectOrderedSet<T> with (T... array) {
		ObjectOrderedSet<T> set = new ObjectOrderedSet<T>();
		set.addAll(array);
		return set;
	}
}
