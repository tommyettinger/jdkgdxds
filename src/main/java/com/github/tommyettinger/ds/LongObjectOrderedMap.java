...
public class LongObjectOrderedMap<V> extends LongObjectMap<V> implements Ordered.OfLong {
	...
	private void resetTo(LongObjectOrderedMap<? extends V> map) {
	...
	public void ensureCapacity(int additionalCapacity) {
		int tableSize = Utilities.tableSize(size + additionalCapacity, loadFactor);
	...
	/**
	 * Sorts this LongObjectOrderedMap in-place by the given LongComparator used on the keys. If {@code comp} is null, then this
	...
	/**
	 * Sorts this LongObjectOrderedMap in-place by the given Comparator used on the values. {@code comp} must
	 * be able to compare {@code V} values. If any null values are present in this LongObjectOrderedMap, then comp
	...
	/**
	 * Appends to an Appendable CharSequence from the contents of this LongObjectOrderedMap, but uses the given {@link LongAppender} and
	 ...
	 * so that they can be read back as Java source code, use {@link LongAppender#READABLE} for the keyAppender.
	 *
	...
	public static class OrderedMapEntries<V> extends Entries<V> {
		...
		public LongObjectMap<V> appendInto(LongObjectMap<V> map) {
			...
			while (iter.hasNext) {
				long k = keys.get(iter.nextIndex);
	...
	public static <V> LongObjectOrderedMap<V> with() {
		return new LongObjectOrderedMap<>(0);
	}
	...
	public static <V> LongObjectOrderedMap<V> with(Number key0, V value0) {
		LongObjectOrderedMap<V> map = new LongObjectOrderedMap<>(1);
	...
	public static <V> LongObjectOrderedMap<V> with(Number key0, V value0, Number key1, V value1) {
		LongObjectOrderedMap<V> map = new LongObjectOrderedMap<>(2);
		map.put(key0.longValue(), value0);
	...
	public static <V> LongObjectOrderedMap<V> with(Number key0, V value0, Number key1, V value1, Number key2, V value2) {
		LongObjectOrderedMap<V> map = new LongObjectOrderedMap<>(3);
		map.put(key0.longValue(), value0);
		map.put(key1.longValue(), value1);
	...
	public static <V> LongObjectOrderedMap<V> with(Number key0, V value0, Number key1, V value1, Number key2, V value2, Number key3, V value3) {
		LongObjectOrderedMap<V> map = new LongObjectOrderedMap<>(4);
		map.put(key0.longValue(), value0);
		map.put(key1.longValue(), value1);
		map.put(key2.longValue(), value2);
	 ...
	 * {@link #LongObjectOrderedMap(long[], Object[])}, which takes all keys and then all values.
	...
	public static <V> LongObjectOrderedMap<V> parse(String str,
													...
													PartialParser<V> valueParser,
													boolean brackets) {
		LongObjectOrderedMap<V> m = new LongObjectOrderedMap<>();
		if (brackets)
