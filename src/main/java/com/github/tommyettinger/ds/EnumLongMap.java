...
public class EnumLongMap implements Iterable<EnumLongMap.Entry> {
	 ...
	 */
	public EnumLongMap(Enum<?>[] universe) {
	 ...
	 */
	public EnumLongMap(Class<? extends Enum<?>> universeClass) {
	 ...
	 */
	public EnumLongMap(Collection<? extends Enum<?>> keys, OfLong values) {
		this();
		putAll(keys, values);
	}
	 ...
	 * @param values a PrimitiveCollection of long values
	 */
	public void putAll(Collection<? extends Enum<?>> keys, OfLong values) {
		Enum<?> key;
	 ...
	 */
	public void putAll(EnumLongMap map) {
	 ...
	 * {@link LongAppender#DEFAULT}. Separates entries with ", " and separates keys from values with
	 ...
	 * {@link LongAppender#DEFAULT}. Separates entries with {@code entrySeparator} and separates keys from values with
	...
	/**
	 * Makes a String from the contents of this EnumLongMap, but uses the given {@link Appender} and
	 ...
	 * @param valueAppender     a LongAppender, such as {@link LongAppender#DEFAULT}
	 * @return a new String representing this map
	 */
	...
	/**
	 * Appends to an Appendable CharSequence from the contents of this EnumLongMap, using
	 * {@link Appender#ENUM_NAME_APPENDER} and {@link LongAppender#DEFAULT} to append keys and values, respectively.
	...
	/**
	 * Appends to an Appendable CharSequence from the contents of this EnumLongMap, but uses the given
	 * {@link Appender} and {@link LongAppender} to convert each key and each value to a customizable
	...
	public static class Values implements OfLong {
		...
		public int hashCode() {
			...
			while (iter.hasNext) {
				...
				hc = hc * 0x9E3779B97F4A7C15L ^ (v ^ v >>> 32);
			}
			return (int)(hc ^ hc >>> 32);
		}
		...
		public boolean equals(Object other) {
			if (other instanceof OfLong) {
				boolean res = map.size() == ((OfLong) other).size();
				if (res) {
	 ...
	 */
	public static EnumLongMap parse(String str,
