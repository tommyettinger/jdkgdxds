...
/// The remainder of the code is based on FastUtil.
	 ...
	 * the second.
	 */
	private void inPlaceMerge (final int from, int mid, final int to, final ByteComparator comp) {
	 ...
	 */
	private int lowerBound (int from, final int to, final int pos, final ByteComparator comp) {
		int len = to - from;
		byte[] items = this.items;
		while (len > 0) {
			int half = len / 2;
	 ...
	 */
	private int upperBound (int from, final int to, final int pos, final ByteComparator comp) {
		int len = to - from;
		byte[] items = this.items;
		while (len > 0) {
			int half = len / 2;
	...
	public void sort (final int from, final int to, final ByteComparator c) {
		...
		if (from < 0 || from >= size || to > size) {
			throw new UnsupportedOperationException("The given from/to range in ByteList.sort() is invalid.");
		}
		/*
