...
/** An {@link ObjectMapX} that also stores keys in an {@link ArrayList} using the insertion order. Null keys are not allowed. No
...
public class OrderedMapX<K, V> extends ObjectMapX<K, V> implements Serializable {
	...
	public <T extends K> void putAll (OrderedMapX<T, ? extends V> map) {
	...
	static public class OrderedMapEntries<K, V> extends Entries<K, V> {
		...
		public OrderedMapEntries (OrderedMapX<K, V> map) {
			keys = map.keys;
	...
	static public class OrderedMapKeys<K> extends Keys<K> {
		...
		public OrderedMapKeys (OrderedMapX<K, ?> map) {
			keys = map.keys;
			iter = new MapIterator<K, Object, K>((OrderedMapX) map) {
				@Override
				...
				public void remove () {
					if (currentIndex < 0) throw new IllegalStateException("next must be called before remove.");
					((OrderedMapX)map).removeIndex(currentIndex);
	...
	static public class OrderedMapValues<V> extends Values<V> {
		...
		public OrderedMapValues (OrderedMapX<?, V> map) {
			keys = map.keys;
