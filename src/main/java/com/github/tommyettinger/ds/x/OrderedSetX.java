...
/** A {@link ObjectSetX} that also stores keys in an {@link ArrayList} using the insertion order. Null keys are not allowed. No
...
public class OrderedSetX<T> extends ObjectSetX<T> implements Serializable {
	...
	static public class OrderedSetIterator<K> extends ObjectSetIterator<K> {
		...
		public OrderedSetIterator (OrderedSetX<K> set) {
			super(set);
			items = set.items;
		}
		...
		public void remove () {
			...
			nextIndex--;
			((OrderedSetX)set).removeIndex(nextIndex);
		}
	}

	@SafeVarargs
	static public <T> OrderedSetX<T> with (T... array) {
		OrderedSetX<T> set = new OrderedSetX<T>();
		set.addAll(array);
		return set;
	}
}
