...
public class ObjectLongOrderedMap<K> extends ObjectLongMap<K> implements Ordered<K> {
	 ...
	 */
	public ObjectLongOrderedMap(OrderType ordering) {
	 ...
	 * @param ordering        determines what implementation {@link #order()} will use
	 */
	public ObjectLongOrderedMap(int initialCapacity, OrderType ordering) {
	 ...
	 * @param ordering        determines what implementation {@link #order()} will use
	 */
	public ObjectLongOrderedMap(int initialCapacity, float loadFactor, OrderType ordering) {
	 ...
	 * @param ordering determines what implementation {@link #order()} will use
	 */
	public ObjectLongOrderedMap(ObjectLongMap<? extends K> map, OrderType ordering) {
	 ...
	 * @param ordering determines what implementation {@link #order()} will use
	 */
	public ObjectLongOrderedMap(ObjectLongOrderedMap<? extends K> other, int offset, int count, OrderType ordering) {
	 ...
	 * @param ordering determines what implementation {@link #order()} will use
	 */
	public ObjectLongOrderedMap(K[] keys, long[] values, OrderType ordering) {
	 ...
	 * @param ordering determines what implementation {@link #order()} will use
	 */
	public ObjectLongOrderedMap(Collection<? extends K> keys, PrimitiveCollection.OfLong values, OrderType ordering) {
	...
	private void resetTo(ObjectLongOrderedMap<? extends K> map) {
	...
	/**
	 * Appends to an Appendable CharSequence from the contents of this ObjectLongOrderedMap, but uses the given {@link Appender} and
	 ...
	 * so that they can be read back as Java source code, use {@link LongAppender#READABLE} for the valueAppender.
	 *
	 ...
	 * @param valueAppender     a LongAppender that can take a long value
	 ...
	 */
	@Override
	public <S extends CharSequence & Appendable> S appendTo(S sb, String entrySeparator, String keyValueSeparator, boolean braces, Appender<K> keyAppender, LongAppender valueAppender) {
		try {
			if (size == 0) {
				if (braces) sb.append("{}");
				return sb;
