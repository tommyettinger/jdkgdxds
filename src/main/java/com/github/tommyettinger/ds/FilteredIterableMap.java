...
public class FilteredIterableMap<K, I extends Iterable<K>, V> extends ObjectObjectMap<I, V> {
	...
	public static <T, I extends Iterable<T>, V> FilteredIterableMap<T, I, V> with (ObjPredicate<T> filter, ObjToSameFunction<T> editor, I key, V value) {
		FilteredIterableMap<T, I, V> map = new FilteredIterableMap<>(filter, editor, 1);
		map.put(key, value);
		return map;
	}
