...
public class NumberedSet<T> implements Set<T>, Ordered<T> {
	...
	/**
	 * Returns an index &gt;= 0 and &lt;= {@code map.mask} for the specified {@code item}, mixed.
	 * <p>
	 ...
	 * and makes an unsigned right shift by {@code map.shift} before casting to int and returning.
	 * This can be overridden to hash {@code item} differently, though all implementors must
	 * ensure this returns results in the range of 0 to {@code map.mask}, inclusive. If nothing
	 * else is changed, then unsigned-right-shifting an int or long by {@code map.shift} will also
	 ...
	 */
	protected int place (Object item) {
		return (int)(item.hashCode() * 0x9E3779B97F4A7C15L >>> map.shift);
