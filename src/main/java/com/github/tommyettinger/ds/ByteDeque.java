...
public class ByteDeque implements PrimitiveCollection.OfByte, Arrangeable {
	...
	protected transient @Nullable ByteDequeIterator iterator1;
	protected transient @Nullable ByteDequeIterator iterator2;

	protected transient @Nullable ByteDequeIterator descendingIterator1;
	...
	/**
	 * Creates a new ByteDeque which can hold the specified number of values without needing to resize backing array.
	 */
	 ...
	 */
	public ByteDeque (ByteIterator coll) {
		this();
		addAll(coll);
	}
	 ...
	 *
	 * @param item a byte to add to the tail
	 * @see #addFirst(byte)
	 */
	 ...
	 *
	 * @param item a byte to add to the head
	 * @see #addLast(byte)
	 */
	...
	/**
	 * This is an alias for {@link #add(int, byte)} to improve compatibility with primitive lists.
	 *
	 * @param index   index at which the specified element is to be inserted
	 * @param element element to be inserted
	 */
	public boolean insert (int index, byte element) {
	 ...
	 */
	public ByteDequeIterator descendingIterator () {
	...
	public boolean equals (Object o) {
		if (this == o)
			return true;
		if (!(o instanceof ByteDeque))
			return false;

		ByteDeque q = (ByteDeque)o;
		final int size = this.size;

		if (q.size != size)
			return false;

		final byte[] myValues = this.values;
		final int myBackingLength = myValues.length;
		final byte[] itsValues = q.values;
		final int itsBackingLength = itsValues.length;

		int myIndex = head;
		int itsIndex = q.head;
		for (int s = 0; s < size; s++) {
			byte myValue = myValues[myIndex];
			byte itsValue = itsValues[itsIndex];
	...
	/**
	 * Sorts this deque in-place using {@link Arrays#sort(byte[], int, int)}.
