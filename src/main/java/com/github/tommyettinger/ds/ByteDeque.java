...
/**
 * A resizable, insertion-ordered double-ended queue of primitive {@code byte} with efficient add and remove at the
 * beginning and end. This extends {@link ByteList} and supports {@link RandomAccess}. Like ByteList, it is a
 * {@link OfByte}, {@link Arrangeable}, and {@link Ordered.OfByte}.
 ...
 * {@link #get(int)} and {@link #set(int, byte)}. Relative to a {@link ByteList}, {@link #get(int)} has slightly
 ...
 * except in some cases where the ByteDeque is empty and an item from it is required. An exception is in
 * {@link #set(int, byte)}, which allows prepending by setting a negative index, or appending by setting a too-large
 ...
 * head or tail region of the deque, such as {@link #addAllFirst(OfByte)} and {@link #truncateFirst(int)}. There are
 ...
 * to {@link ByteList} as long as the performance of {@link #get(int)} is adequate.
...
public class ByteDeque extends ByteList implements RandomAccess, Arrangeable, PrimitiveCollection.OfByte, Ordered.OfByte {
	 ...
	 *
	 * @param coll a Collection of byte that will be copied into this and used in full
	 * @throws NullPointerException if {@code coll} is {@code null}
	 */
	public ByteDeque(OfByte coll) {
		this(coll.size());
		addAll(coll);
	}
	 ...
	 *
	 * @param a an array of byte that will be copied into this and used in full
	...
	public void addLast(byte value1, byte value2, byte value3, byte value4, byte value5) {
		byte[] items = this.items;
	...
	public void addLast(byte value1, byte value2, byte value3, byte value4, byte value5, byte value6) {
		byte[] items = this.items;
	...
	public void addFirst(byte value1, byte value2, byte value3, byte value4, byte value5) {
		byte[] items = this.items;
	...
	public void addFirst(byte value1, byte value2, byte value3, byte value4, byte value5,
	                     byte value6, byte value7, byte value8) {
		byte[] items = this.items;
	...
	/**
	 * Trims the capacity of this {@code ByteDeque} instance to be the
	 * deque's current size.  An application can use this operation to minimize
	 * the storage of a {@code ByteDeque} instance.
	 */
	...
	protected int ensureGap(int index, int gapSize) {
		...
		if (size == 0) {
			...
			if (items.length < gapSize + size) {
				byte item = this.items[head];
				this.items = new byte[gapSize + size];
				if (index == 0) {
	...
	/**
	 * An alias for {@link #addAll(int, OfByte)}; inserts all elements
	...
	/**
	 * Like {@link #addAll(int, OfByte)}, but takes an array instead of a PrimitiveCollection.OfByte, gets items starting at
	 ...
	 * (or equivalently, if this deque changed as a result of the call).
	 *
	 * <p>This method is equivalent to {@link #removeFirstOccurrence(byte)}.
	 *
	 * @param o element to be removed from this deque, if present
	 * @return {@code true} if an element was removed as a result of this call
	 */
	 ...
	 * Like {@link ArrayList} or {@link ByteList}, but unlike {@link LinkedList}, this runs in O(1) time.
	 * It is expected to be slightly slower than {@link ByteList#get(int)}, which also runs in O(1) time.
	 * Unlike get() in ArrayList or ByteList, this considers negative indices to refer to the first item, and
	 ...
	 * deque, this delegates to {@link #addLast(byte)} and returns {@link #getDefaultValue() the default value}.
	 * If the index is negative, this delegates to {@link #addFirst(byte)} and returns
	 ...
	 * other subtypes of {@link ByteList}.
	 * If {@code o} is not a ByteList
	 ...
	 * use the natural ordering for byte.
	 *
	...
	/**
	 * A {@link ByteIterator} over the elements of a ByteDeque.
	 */
