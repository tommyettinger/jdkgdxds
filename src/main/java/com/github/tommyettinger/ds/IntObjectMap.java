...
public class IntObjectMap<V> implements Iterable<IntObjectMap.Entry<V>> {
	...
	private void resetTo(IntObjectMap<? extends V> map) {
	 ...
	 * convert its key to a primitive int, regardless of which Number type was used.
	 *
	 ...
	 * Number, such as {@link Integer} or {@link Double}, and will be converted to primitive {@code int}s. Any
	...
	public static <V> IntObjectMap<V> parse(String str,
											...
											PartialParser<V> valueParser,
											boolean brackets) {
		IntObjectMap<V> m = new IntObjectMap<>();
		if (brackets)
