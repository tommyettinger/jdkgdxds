...
public class LongFloatMap implements Iterable<LongFloatMap.Entry> {
	...
	private void resetTo(LongFloatMap map) {
	...
	/**
	 * Appends to an Appendable CharSequence from the contents of this LongFloatMap. Uses
	 * {@link LongAppender#DEFAULT} and {@link FloatAppender#DEFAULT} to write keys and values, and appends them
	...
	/**
	 * Appends to an Appendable CharSequence from the contents of this LongFloatMap, but uses the given {@link LongAppender} and
	 * {@link FloatAppender} to convert each key and each value to a customizable representation and append them
	 * to {@code sb}. These functions are often method references to methods in Base, such as
	 * {@link Base#appendReadable(CharSequence, long)} and {@link Base#appendUnsigned(CharSequence, long)}. To use
	 * the default String representation, you can use {@link LongAppender#DEFAULT} or {@link FloatAppender#DEFAULT}
	...
	public static class Entries extends AbstractSet<Entry> implements EnhancedCollection<Entry> {
		...
		/**
		 * Append the remaining items that this can iterate through into the given LongFloatMap.
		 * Does not change the position of this iterator.
		 *
		 * @param map a modifiable LongFloatMap; may have items appended into it
		 * @return the given map
		 */
		public LongFloatMap appendInto(LongFloatMap map) {
			EntryIterator iter = iterator();
			while (iter.hasNext) {
	 ...
	 * {@link #LongFloatMap(long[], float[])}, which takes all keys and then all values.
	...
	/**
	 * Adds items to this map drawn from the result of {@link #toString(String)} or
	 * {@link #appendTo(CharSequence, String, String, boolean, LongAppender, FloatAppender)}. Each item can vary
