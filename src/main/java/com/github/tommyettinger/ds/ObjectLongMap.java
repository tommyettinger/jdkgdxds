...
/**
 * An unordered map where the keys are objects and the values are unboxed longs. Null keys are not allowed. No allocation is done except
...
//		return item.hashCode() & mask;
	...
	/**
	 * Appends to an Appendable CharSequence from the contents of this ObjectLongMap, using
	 * {@code Appender::append} and {@link LongAppender#DEFAULT} to append keys and values, respectively.
	...
	/**
	 * Appends to an Appendable CharSequence from the contents of this ObjectLongMap, but uses the given {@link Appender} and
	 * {@link LongAppender} to convert each key and each value to a customizable representation and append them
	...
	public static class Entries<K> extends AbstractSet<Entry<K>> implements EnhancedCollection<Entry<K>> {
		...
		public ObjectList<Entry<K>> toList() {
			ObjectList<Entry<K>> list = new ObjectList<>(map.size);
	...
	public static class Keys<K> extends AbstractSet<K> implements EnhancedCollection<K> {
		...
		public ObjectList<K> toList() {
			...
			KeyIterator<K> iter = new KeyIterator<>(map);
			while (iter.hasNext) {
				list.add(iter.next());
			}
			return list;
	 ...
	 * {@link #ObjectLongMap(Object[], long[])}, which takes all keys and then all values.
	 ...
	 * @param keyParser         a PartialParser that returns a {@code K} key from a section of {@code str}
	 */
	public static <K> ObjectLongMap<K> parse(String str,
	...
	public static <K> ObjectLongMap<K> parse(String str,
											 ...
											 PartialParser<K> keyParser,
											 boolean brackets) {
		ObjectLongMap<K> m = new ObjectLongMap<>();
		if (brackets)
