...
public class EnumIntOrderedMap extends EnumIntMap implements Ordered<Enum<?>> {
	 ...
	 */
	public void putAll(@NonNull ObjectIntOrderedMap<Enum<?>> map) {
		for (int i = 0, kl = map.size(); i < kl; i++) {
			put(map.keyAt(i), map.getAt(i));
		}
	}
	 ...
	 * <p>Note that the same PrimitiveCollection.OfInt instance is returned each time this method is called. Use the
