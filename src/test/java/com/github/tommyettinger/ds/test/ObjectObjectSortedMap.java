/*
 * Copyright (c) 2017-2025 See AUTHORS file.
 *
 ...
 * @author Robin Wang
 */
public class ObjectObjectSortedMap<K, V> extends AbstractMap<K, V> implements SortedMap<K, V> {
	/**
	 ...
	 */
	public ObjectObjectSortedMap(Comparator<? super K> comparator) {
	 ...
	 * @param update   the update array which is used by {@link ObjectObjectSortedMap#put(Object, Object)}
	...
	private int randomLevel() {
		// Marsaglia Xorshift generator to change state
		random ^= random << 13;
		random ^= random >>> 17;
		random ^= random << 5;
		// Always returns at least 1, half the time at least 2, a quarter of the time at least 3...
		return Integer.numberOfLeadingZeros(random) + 1;
	}
	...
	private class EntrySet extends AbstractSet<Entry<K, V>> {
		...
		@Override
		public int size() {
			return ObjectObjectSortedMap.this.size();
		}
	}
	...
	private static class SortedSubMap<K, V> extends AbstractMap<K, V> implements SortedMap<K, V>, Serializable {
		...
		final ObjectObjectSortedMap<K, V> map;
		...
		SortedSubMap(ObjectObjectSortedMap<K, V> map, boolean fromStart, K low, boolean toEnd, K high) {
			if (!fromStart && !toEnd) {
