...
public class CaseInsensitiveIntMap6 implements Iterable<CaseInsensitiveIntMap6.Entry> {
	public int size;
	...
	public CaseInsensitiveIntMap6() {
		this(51, 0.6f);
	}

	/** Creates a new map with a load factor of 0.6 .
	 * @param initialCapacity The backing array size is initialCapacity / loadFactor, increased to the next power of two. */
	public CaseInsensitiveIntMap6(int initialCapacity) {
	 ...
	 * @param initialCapacity The backing array size is initialCapacity / loadFactor, increased to the next power of two. */
	public CaseInsensitiveIntMap6(int initialCapacity, float loadFactor) {
	 ...
	 * 0.6 . */
	public CaseInsensitiveIntMap6(String[] keys, int[] values) {
	...
	public CaseInsensitiveIntMap6(CaseInsensitiveIntMap6 map) {
	...
	public void putAll (CaseInsensitiveIntMap6 map) {
	...
	public void clear () {
		System.out.println("Revision 6 map gets total collisions: " + collisionTotal + ", PILEUP: " + longestPileup);
...
//		longestPileup = 0;
	...
	public boolean equals (Object obj) {
		...
		if (!(obj instanceof CaseInsensitiveIntMap6)) return false;
		CaseInsensitiveIntMap6 other = (CaseInsensitiveIntMap6)obj;
	...
	private static class MapIterator {
		...
		final CaseInsensitiveIntMap6 map;
		int nextIndex, currentIndex;
		boolean valid = true;

		public MapIterator (CaseInsensitiveIntMap6 map) {
			this.map = map;
			reset();
		}
	...
	public static class Entries extends MapIterator implements Iterable<Entry>, Iterator<Entry> {
		...
		public Entries (CaseInsensitiveIntMap6 map) {
			super(map);
		}
	...
	public static class Values extends MapIterator {
		public Values (CaseInsensitiveIntMap6 map) {
			super(map);
		}
	...
	public static class Keys extends MapIterator implements Iterable<String>, Iterator<String> {
		public Keys (CaseInsensitiveIntMap6 map) {
			super(map);
		}
	...
	public static int hashCodeIgnoreCase (final CharSequence data, int seed) {
		...
		if(len<=2){
			...
			if(i>=6){
				...
				do{
					seed=Compatibility.imul(b1, Category.caseUp(data.charAt(p  )) << 11 ^ Category.caseUp(data.charAt(p+1))) - (seed<<24|seed>>> 8);
					see1=Compatibility.imul(b2, Category.caseUp(data.charAt(p+2)) << 11 ^ Category.caseUp(data.charAt(p+3))) - (see1<<15|see1>>>19);
					see2=Compatibility.imul(b3, Category.caseUp(data.charAt(p+4)) << 11 ^ Category.caseUp(data.charAt(p+5))) - (see2<< 7|see2>>>25);
					p+=6;i-=6;
				}while(i>=6);
				seed^=see1^see2;
			}
			while((i>2)){
				seed=Compatibility.imul(b0, Category.caseUp(data.charAt(p  )) << 11 ^ Category.caseUp(data.charAt(p+1))) - (seed<< 3|seed>>>29);
				i-=2; p+=2;
			}
