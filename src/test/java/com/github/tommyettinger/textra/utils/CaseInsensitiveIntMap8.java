...
public class CaseInsensitiveIntMap8 implements Iterable<CaseInsensitiveIntMap8.Entry> {
	public int size;
	...
	public CaseInsensitiveIntMap8() {
		this(51, 0.6f);
	}

	/** Creates a new map with a load factor of 0.6 .
	 * @param initialCapacity The backing array size is initialCapacity / loadFactor, increased to the next power of two. */
	public CaseInsensitiveIntMap8(int initialCapacity) {
	 ...
	 * @param initialCapacity The backing array size is initialCapacity / loadFactor, increased to the next power of two. */
	public CaseInsensitiveIntMap8(int initialCapacity, float loadFactor) {
	 ...
	 * 0.6 . */
	public CaseInsensitiveIntMap8(String[] keys, int[] values) {
	...
	public CaseInsensitiveIntMap8(CaseInsensitiveIntMap8 map) {
	...
	public void putAll (CaseInsensitiveIntMap8 map) {
	...
	public void clear () {
		System.out.println("Revision 8 map gets total collisions: " + collisionTotal + ", PILEUP: " + longestPileup);
...
//		longestPileup = 0;
	...
	public boolean equals (Object obj) {
		...
		if (!(obj instanceof CaseInsensitiveIntMap8)) return false;
		CaseInsensitiveIntMap8 other = (CaseInsensitiveIntMap8)obj;
	...
	private static class MapIterator {
		...
		final CaseInsensitiveIntMap8 map;
		int nextIndex, currentIndex;
		boolean valid = true;

		public MapIterator (CaseInsensitiveIntMap8 map) {
			this.map = map;
			reset();
		}
	...
	public static class Entries extends MapIterator implements Iterable<Entry>, Iterator<Entry> {
		...
		public Entries (CaseInsensitiveIntMap8 map) {
			super(map);
		}
	...
	public static class Values extends MapIterator {
		public Values (CaseInsensitiveIntMap8 map) {
			super(map);
		}
	...
	public static class Keys extends MapIterator implements Iterable<String>, Iterator<String> {
		public Keys (CaseInsensitiveIntMap8 map) {
			super(map);
		}
	 ...
	 *
	 * @param s a non-null CharSequence; often a String, but this has no trouble with a StringBuilder
	 * @param seed any int; must be the same between calls if two equivalent values for {@code data} must be the same
	 * @return an int hashCode; quality should be similarly good across any bits
	 */
	public static int hashCodeIgnoreCase (final CharSequence s, int seed) {
		if(s == null) return 0;
		final int hm = 0xF45, len = s.length();
		int hash = seed;
		for (int i = 0; i < len; i++) {
			hash = ((hash << 13 | hash >>> 19) * hm) ^ Casing.caseUp(s.charAt(i));
		}
		hash *= hm;
